...
class Builder:
    def __init__(self, name: str, ctx: dict):
        ...
        if KEY_TYPE in ctx.keys():
            self.context = dict_writer(g_default_context[ctx[KEY_TYPE]], ctx, True, True)
        else:
            self.context = dict_writer(g_default_context['executable'], ctx, True, True)

        self.sources = self.get_all_sources()
        self.output = self.context[KEY_OUTPUT]

    def get_all_sources(self) -> list:
        return glob.glob(f"{self.context[KEY_FOLDERS][KEY_FOLDERS_SOURCE]}/**/*.cpp", recursive=True)

    def get_dependencies(self, dfile) -> list:
        if not os.path.exists(dfile):
            return None
    
        with open(dfile, mode='r') as fs:
            tmp = fs.read()
    ...
    def is_compile_needed(self, path) -> bool:
        if not os.path.exists(self.as_object_path(path)):
            return True
    ...
    def as_depend_path(self, path):
        return self.context[KEY_FOLDERS][KEY_FOLDERS_BUILD] + '/' + \
            path[len(self.context[KEY_FOLDERS][KEY_FOLDERS_SOURCE]) + 1: path.rfind('.')] + '.d'

    def as_object_path(self, path):
        return self.context[KEY_FOLDERS][KEY_FOLDERS_BUILD] + '/' + \
            path[len(self.context[KEY_FOLDERS][KEY_FOLDERS_SOURCE]) + 1: path.rfind('.')] + '.o'
    ...
    def compile(self, path):
        ...
        if '/' in path:
            dirs = path.split('/')[:-1]
            os.system(f'mkdir -p {self.context[KEY_FOLDERS][KEY_FOLDERS_BUILD]}/{"/".join(dirs)}')
    ...
    def build(self) -> bool:
        ...
        if not os.path.exists(self.output) or self.is_compiled:
            self.link()
        else:
